...
from rag import embedding_utils, retrieval_strategies
...
class PromptSelector:
    ...
    def convert_string_to_conversation(
...
class PromptManager:
    ...
    def set_retrieval_strategy(
        self,
        retrieval_strategy: retrieval_strategies.RetrievalStrategy,
    ) -> PromptManager:
    ...
    def build_query(
        ...
        for message in messages[::-1]:
            ...
            if len(expected_slots) > 0:
                ...
                assert len(slot_fill_dict) == len(
                    expected_slots,
                ), "Unexpected fill provided."
                ...
                try:
                    ...
                    raise KeyError(
                        f"Failed to fill {expected_slots} with {slot_fill_dict}.",
                    )
            else:
    ...
    def compute_stored_token_counts(self) -> int:
        token_counts = embedding_utils.get_token_counts(
            [message["content"] for message in self.stored_messages],
        )
