...
from rag import embedding_utils
...
class RetrievalDb:
    ...
    def create_embeddings(self):
        embedding_list = embedding_utils.batch_embed_texts(
            self.df[self.embed_col],
            self.df[self.n_tokens_col],
        )
        self.embedding_mat = np.concatenate(
            [e.reshape(1, -1) for e in embedding_list],
            axis=0,
        )
    ...
    def load(self):
        if not self.df_filepath.exists():
            raise ValueError(
                f"Trying to load a dataframe from non-existent path: {self.df_filepath}",
            )
    ...
    def compute_string_distances(self, query_str: str) -> np.array:
        embedding_list = embedding_utils.get_openai_embeddings(
            [normalize_text(query_str)],
        )
